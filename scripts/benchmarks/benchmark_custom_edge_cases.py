"""
MindGuard - Custom Edge Cases Testing & Reporting Benchmark
------------------------------------------------------------
...
Usage:
    python scripts/benchmarks/benchmark_custom_edge_cases.py [optional_path_to_json]
"""
...
from services.crisis_rules import (
    evaluate_crisis_pipeline, evaluate_deterministic_crisis, is_contextual_or_negated
)
...
def run_edge_case_benchmark(json_filepath: str = None):
    ...
    if json_filepath:
        ...
        if target_path.exists():
            ...
            print(" -> Falling back to built-in edge cases...\n")
    else:
    ...
    for idx, case in enumerate(edge_cases, 1):
        ...
        # Evaluate unified crisis detection pipeline
        pipeline_res = evaluate_crisis_pipeline(text)
        is_crisis = bool(pipeline_res["is_crisis"])
        method = f"Pipeline ({pipeline_res.get('source', 'unknown')})"
