...
CRISIS_EXPLICIT_PATTERNS_RAW: List[str] = [
    ...
    r"\b(?:drank|swallowed|ingested|took|downed|drinking)\s+(?:a\s+)?(?:bottle\s+of\s+|bunch\s+of\s+|handful\s+of\s+)?(?:bleach|poison|chemicals|toxic\s+substances?)\b",
    r"\b(?:took|swallowed|ingested|downed|popped)\s+(?:\d+|all|a\s+lot\s+of|a\s+handful\s+of|the\s+whole\s+bottle\s+of|the\s+entire\s+bottle\s+of|my\s+whole\s+prescription\s+of)\s+(?:sleeping\s+)?(?:pills|tablets|capsules|meds|medication|painkillers)\b",
    ...
    r"\bno\s+big\s+deal,?\s*just\s+ready\s+to\s+die\b",
    ...
    r"\bapne\s+aap\s+ko\s+khatam\s+karne\s+(?:laga|lagi|wala|wali)\s+(?:hoon|hun)\b",
    r"\bkhud\s*kushi\s+karne\s+(?:laga|lagi|wala|wali)\s+(?:hoon|hun)\b",
    r"\b(?:maine?\s+)?marne\s+ka\s+waqt\s+a+a?gaya\s*(?:hai)?\b",
    r"\b(?:mujhe\s+)?zinda\s+nah?i\s+rehna(?:\s+chaht[ai]\s+(?:hoon|hun))?\b",
    r"\bmain\s+mar\s+ja+na(?:\s+chaht[ai]\s+(?:hoon|hun))?\b",
]
